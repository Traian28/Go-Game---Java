...
fichaComputadora(X,Y,[]):-generarPosicion(1,1,X,Y),assertB(piedra(b,X,Y)),noHaySuicidio(b,X,Y,_,_),actualizarInvalidas(0,0,_).
fichaComputadora(_X,_Y,[]):-pasaJugador(b).
